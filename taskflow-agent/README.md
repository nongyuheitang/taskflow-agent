...
这个项目展示了以下 **前端开发工程师（FDE）** 核心能力：

| 能力维度 | 体现 |
|---------|------|
| **组件化思维** | 10 个组件职责清晰、可复用、接口明确 |
| **状态管理** | Zustand 单一数据源 + 派生计算，避免状态冗余 |
| **TypeScript** | 泛型、联合类型、类型守卫、接口设计 |
| **CSS 工程化** | CSS 变量体系、三栏响应式布局、动画细节 |
| **产品思维** | 空状态提示、Toast 反馈、确认对话框、数据持久化 |
| **AI 集成意识** | 预留 LLM 接入点、规则引擎与 AI 的权衡 |
| **文档能力** | README 包含架构图、面试讲解词、扩展路线 |
| **工程规范** | 代码即文档、命名清晰、文件结构自解释 |

## 7. Demo 展示流程（面试 5 分钟）

1. **开场 (30s)**：打开应用 → 展示三栏布局 → 说明「这不是普通 Todo List」
2. **数据加载 (30s)**：点击「示例数据」→ 展示 8 条面试相关任务
3. **看板视图 (30s)**：切换到看板 → 拖拽一张卡片从 todo 到 doing
4. **任务详情 (30s)**：点击任务卡片 → 右侧出现详情 → 子任务进度条
5. **AI 拆解 (60s)**：点击「AI 拆解」→ 输入「准备前端实习面试」→ 展示拆解结果和 reasoning
6. **Today Plan (30s)**：右侧面板展示今日推荐 Top 3，说明推荐算法
7. **面试讲解 (60s)**：切换到「面试讲解」标签 → 快速过一遍技术架构和 FDE 价值
8. **收尾 (30s)**：数据统计 → 导出 JSON → 说明扩展方向

## 8. 未来扩展方向

### 接入 Claude API
- 将 `decomposeTask()` 替换为 Claude API 调用
- 支持自然语言创建任务（"明天下午提醒我提交周报 #高优先级"）

### Cherry Studio / MCP
- 导出 MCP Server 接口，让 Claude Desktop 直接管理任务
- 接入 Cherry Studio 知识库，根据项目文档自动生成任务
...
> "这个项目叫 TaskFlow Agent，是一个融合了 AI 辅助拆解的任务管理工作台。它的核心定位不是普通 Todo List，而是把模糊需求变成可执行任务流的思维工具。
>
> 技术上用了 Vite + React + TypeScript，状态管理选了 Zustand 而不是 Redux，因为它更轻量且 selector 天然防重渲染。所有数据通过 localStorage 持久化，不需要后端依赖。
>
> 比较有意思的是 AI 拆解模块——我用规则引擎做了一个分层匹配系统：先尝试自然语言分隔符拆分，再匹配领域关键词，再匹配动作模式，最后兜底通用模板。每条结果都带 reasoning 字段解释匹配逻辑。这个函数接口设计成纯函数，未来只要替换返回值就能接入真正的 LLM。
>
> 代码组织上，types.ts 集中管理类型定义，lib 层全是纯函数方便测试，组件职责单一。样式用了 CSS 变量体系，右侧深色侧栏 + 三栏布局的设计是想做成现代 AI 工作台的风格。
>
> 如果你对某个部分想深入了解，我可以展开讲——比如 Zustand 的 slice 模式、AI 拆解引擎的匹配策略、或者 Today Plan 的推荐算法。"
